...
type
  TFunc2DoubleBool = function(a, b: double): boolean;
...
type
  TIterateFuncTriple = class(TObject)
  protected
  public
...
type
  TIterateFuncPair = class(TObject)
  protected
  public
...
type
  TIBetaSeriesIter = class(TIterateFunc)
  protected
    FRes : double;
    Fx   : double;
    Fapn : double;
...
type
  TLowerIncompleteGammaSeriesIter = class(TIterateFunc)
  protected
    Fa  : double;
    Fz  : double;
    FRes: double;
...
type
  TSmallGamma2SeriesIter = class(TIterateFunc)
  protected
    Fa  : double;
    Fx  : double;
    Fapn: double;
...
type
  TErfAsymptSeriesIter = class(TIterateFunc)
  protected
    FRes: double;
    Fxx : double;
    Ftk : integer;
...
type
  DistributionQuantileIter = class(TIterateFunc)
  protected
    FComp            : boolean;
    FTarget          : double;
    Fn               : double;
...
type
  TIBetaFraction2Iter = class(TIterateFuncPair)
  protected
    Fa: double;
    Fb: double;
    Fx: double;
...
type
  TUpperIncompleteGammaFractIter = class(TIterateFuncPair)
  protected
    Fa: double;
    Fz: double;
    Fk: integer;
...
type
  TTemmeRootFinderIter = class(TIterateFuncPair)
  protected
    Ft: double;
    Fa: double;
  public
...
type
  TGammaPInverseIter = class(TIterateFuncTriple)
  protected
    Fa  : double;
    Fp  : double;
    FInv: boolean;
...
type
  TIBetaRootsIter = class(TIterateFuncTriple)
  protected
    Fa     : double;
    Fb     : double;
    FTarget: double;
...
type
  TXLSLanczos13m53 = class(TXLSLanczos)
  protected
  public
...
begin
  ...
  // Special cases:
  if ((c = a) and (b < const_epsilon)) then
  begin
    Result := TGamma(b);
    Exit;
  end
  else if ((c = b) and (a < const_epsilon)) then
  begin
    Result := TGamma(a);
    Exit;
  end;
...
begin
  ...
  // end
  // end
  { else } if (Abs(d * d * a) <= 100) and (a > 150) then
  begin
  ...
  begin
    ...
    alz := a * Ln(z / agh);
    amz := a - z;
    if (min(alz, amz) <= LogMinValue) or (max(alz, amz) >= LogMaxValue) then
    begin
      amza := amz / a;
      if (min(alz, amz) / 2 > LogMinValue) and (max(alz, amz) / 2 < LogMaxValue) then
      begin
        // compute square root of the result and then square it:
        sq     := Power(z / agh, a / 2) * exp(amz / 2);
        prefix := sq * sq;
      end
      else if (min(alz, amz) / 4 > LogMinValue) and (max(alz, amz) / 4 < LogMaxValue) and (z > a) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      //
      if ((z <= MAX_QUICKFACTORIAL) and (z + delta <= MAX_QUICKFACTORIAL)) then
      begin
...
begin
  ...
  Result := 0;
  if (z < const_epsilon) then
  begin
    Result := -Ln(z);
    Exit;
  end
  else if ((zm1 = 0) or (zm2 = 0)) then
  begin
...
begin
  ...
  if (not invert and (z > MaxSingle)) then
  begin
...
function IGammaTemmeLarge(a, x: double): double;
const
  C0: array [0 .. 6] of double = (-0.333333333, 0.0833333333, -0.0148148148, 0.00115740741, 0.000352733686, -0.000178755144, 0.391926318E-4);

const
  C1: array [0 .. 4] of double = (-0.00185185185, -0.00347222222, 0.00264550265, -0.000990226337, 0.000205761317);

const
  C2: array [0 .. 2] of double = (0.00413359788, -0.00268132716, 0.000771604938);
var
  sigma    : double;
  phi      : double;
  y        : double;
...
begin
  ...
  if (is_int and is_small_a and (x > 0.6)) then
  begin
  ...
  else if (is_half_int and is_small_a and (x > 0.2)) then
  begin
  ...
  begin
    ...
    //
    use_temme := False;
    if (Normalized and IsSpecialized and (a > 20)) then
    begin
      sigma := Abs((x - a) / a);
      if ((a > 200) and (ResNumDigits <= 113)) then
      begin
        //
...
var
  ...
  begin
    ...
    if (Abs(Denom) < 1) then
    begin
      if (Abs(Denom * MaxDouble) <= Abs(Num)) then
      begin
        Result := r;
        Exit;
      end;
...
begin
  ...
  if ((TolFunc(a, b)) or (Fa = 0) or (Fb = 0)) then
  begin
    max_iter := 0;
    if (Fa = 0) then
      b := a
  ...
  begin
    ...
    if ((Count > 0) and (Fa <> 0) and not TolFunc(a, b)) then
    begin
      //
  ...
  while ((Count > 0) and (Fa <> 0) and not TolFunc(a, b)) do
  begin
    // save our brackets:
    a0 := a;
    b0 := b;
...
begin
  ...
  if (p <= Power(1 - success_fraction, trials)) then
  begin
    Result := 0;
    Exit;
  end;
  ...
  else if (trials < 10) then
  begin
    // pretty inaccurate guess in this area:
    if (guess > trials / 64) then
    begin
      guess  := trials / 4;
      Factor := 2;
    end
...
begin
  ...
  begin
    ...
    else if max(Abs(l1), Abs(l2)) < 0.5 then
    begin
      //
      ...
      small_a := a < b;
      ratio   := b / a;
      if (small_a and (ratio * l2 < 0.1)) or (not small_a and (l1 / ratio > 0.1)) then
      begin
  ...
  begin
    ...
    l1 := a * Ln(b1);
    l2 := b * Ln(b2);
    if (l1 >= LogMaxValue) or (l1 <= LogMinValue) or (l2 >= LogMaxValue) or (l2 <= LogMinValue) then
    begin
...
begin
  ...
  begin
    ...
    begin
      // Both a,b < 1:
      if (a >= min(0.2, b)) or (Power(x, a) <= 0.9) then
      begin
        if not invert then
    ...
    begin
      // One of a, b < 1 only:
      if (b <= 1) or ((x < 0.1) and (Power(b * x, a) <= 0.7)) then
      begin
        if not invert then
          fract := IBetaSeries(a, b, x, 0, Def_Lanczos, Normalized, Derivative, y)
        else
...
begin
  ...
  if ((ResNumDigits <= 36) and has_10_digits) then
  begin
    Result := guess;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    minv := min(a, b);
    maxv := max(a, b);
    if ((sqrt(minv) > (maxv - minv)) and (minv > 5)) then
    begin
      //
    ...
    begin
      ...
      lambda := minv / r;
      if ((lambda >= 0.2) and (lambda <= 0.8) and (lambda >= 10)) then
      begin
        //
  ...
  begin
    ...
    //
    fs := IBeta(a, b, xs) - p;
    if (Abs(fs) / p < const_epsilon * 3) then
    begin
  ...
  if ((x < 1E-50) and ((a < 1) or (b < 1))) then
  begin
    //
...
begin
  ...
  c[3]  := (0.0083333333333333333333 * in_ + 0.066666666666666666667) * in_ + 0.058333333333333333333;
  c[4]  := ((0.00019841269841269841270 * in_ + 0.0017857142857142857143) * in_ + 0.026785714285714285714) * in_ + 0.025198412698412698413;
  c[5]  := (((2.7557319223985890653E-6 * in_ + 0.00037477954144620811287) * in_ - 0.0011078042328042328042) * in_ + 0.010559964726631393298) * in_ + 0.012039792768959435626;
  c[6]  := ((((2.5052108385441718775E-8 * in_ - 0.000062705427288760622094) * in_ + 0.00059458674042007375341) * in_ - 0.0016095979637646304313) * in_ + 0.0061039211560044893378) * in_ + 0.0038370059724226390893;
  c[7]  := (((((1.6059043836821614599E-10 * in_ + 0.000015401265401265401265) * in_ - 0.00016376804137220803887) * in_ + 0.00069084207973096861986) * in_ - 0.0012579159844784844785) * in_ + 0.0010898206731540064873) * in_ + 0.0032177478835464946576;
  c[8]  := ((((((7.6471637318198164759E-13 * in_ - 3.9851014346715404916E-6) * in_ + 0.000049255746366361445727) * in_ - 0.00024947258047043099953) * in_ + 0.00064513046951456342991) * in_ - 0.00076245135440323932387) * in_ + 0.000033530976880017885309) * in_ + 0.0017438262298340009980;
  c[9]  := (((((((2.8114572543455207632E-15 * in_ + 1.0914179173496789432E-6) * in_ - 0.000015303004486655377567) * in_ + 0.000090867107935219902229) * in_ - 0.00029133414466938067350) * in_ + 0.00051406605788341121363) * in_ - 0.00036307660358786885787) * in_ - 0.00031101086326318780412) * in_ + 0.00096472747321388644237;
  c[10] := ((((((((8.2206352466243297170E-18 * in_ - 3.1239569599829868045E-7) * in_ + 4.8903045291975346210E-6) * in_ - 0.000033202652391372058698) * in_ + 0.00012645437628698076975) * in_ - 0.00028690924218514613987) * in_ + 0.00035764655430568632777) * in_ - 0.00010230378073700412687) * in_ - 0.00036942667800009661203) * in_ + 0.00054229262813129686486;
  //
...
begin
  ...
  if (sign(last_f0) * sign(f0) < 0) then
  begin
    if (delta < 0) then
      delta := (res - min) / 2
    else
...
begin
  ...
  repeat
    ...
    if ((convergence > 0.8) and (convergence < 2)) then
    begin
    ...
    begin
      ...
      if (Abs(diff) < 1) then
        diff := 1 / diff;
      if (not out_of_bounds_sentry and (diff > 0) and (diff < 3)) then
      begin
...
begin
  ...
  begin
    g := TGamma(a);
    b := q * g;
    if ((b > 0.6) or ((b >= 0.45) and (a >= 0.3))) then
    begin
    ...
    else if ((a < 0.3) and (b >= 0.35)) then
    begin
      t      := exp(const_euler - b);
      u      := t * exp(t);
      Result := t * exp(u);
    end
    else if ((b > 0.15) or (a >= 0.3)) then
    begin
      y      := -Ln(b);
      u      := y - (1 - a) * Ln(y);
    ...
    begin
      ...
      c5 := (a - 1) * (-(c1_4 / 4) + (11 * a - 17) * c1_3 / 6 + (-3 * a_2 + 13 * a - 13) * c1_2 + (2 * a_3 - 25 * a_2 + 72 * a - 61) * C1 / 2 + (25 * a_3 - 195 * a_2 + 477 * a - 379) / 12);
  ...
  begin
    ...
    if ((a >= 500) and (Abs(1 - w / a) < 1E-6)) then
    begin
      Result         := w;
      has_10_digits^ := True;
    end
    ...
    begin
      ...
      if ((z <= 0.01 * ap1) or (z > 0.7 * ap1)) then
      begin
        Result := z;
        if (z <= 0.002 * ap1) then
          has_10_digits^ := True;
...
begin
  ...
  while ((i < 3) or (j < 5)) do
  begin
    while ((j < 5) and ((Result >= 1) or (i >= 3))) do
    begin
      Result := Result / Denom[j];
      Inc(j);
    end;
    while ((i < 3) and ((Result <= 1) or (j >= 5))) do
    begin
      Result := Result * Num[i];
      Inc(i);
    end
...
begin
  ...
  begin
    ...
    while (diff > IIF(invert, 1, Result) * const_epsilon) do
    begin
  ...
  begin
    ...
    begin
      ...
      Result := HypergeometricDistPDF(x, r, n, N_);
      diff   := Result;
      while ((x <= upper_limit) and (diff > IIF(invert, 1, Result) * const_epsilon)) do
      begin
...
const
  Num: array [0 .. 12] of double = (23531376880.41075968857200767445163675473, 42919803642.64909876895789904700198885093, 35711959237.35566804944018545154716670596, 17921034426.03720969991975575445893111267, 6039542586.35202800506429164430729792107, 1439720407.311721673663223072794912393972, 248874557.8620541565114603864132294232163, 31426415.58540019438061423162831820536287,
    2876370.628935372441225409051620849613599, 186056.2653952234950402949897160456992822, 8071.672002365816210638002902272250613822, 210.8242777515793458725097339207133627117, 2.506628274631000270164908177133837338626);

  Denom: array [0 .. 12] of longword = (0, 39916800, 120543840, 150917976, 105258076, 45995730, 13339535, 2637558, 357423, 32670, 1925, 66, 1);
begin
...
const
  Num: array [0 .. 12] of double = (56906521.91347156388090791033559122686859, 103794043.1163445451906271053616070238554, 86363131.28813859145546927288977868422342, 43338889.32467613834773723740590533316085, 14605578.08768506808414169982791359218571, 3481712.15498064590882071018964774556468, 601859.6171681098786670226533699352302507, 75999.29304014542649875303443598909137092,
    6955.999602515376140356310115515198987526, 449.9445569063168119446858607650988409623, 19.51992788247617482847860966235652136208, 0.5098416655656676188125178644804694509993, 0.00606184234624890652578375396455593688322);
